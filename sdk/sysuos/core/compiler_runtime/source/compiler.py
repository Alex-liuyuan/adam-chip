...
def emit_aeg(path: Path, plan: dict[str, object]) -> dict[str, object]:
    backend = plan["selected"]["segments"][0]["backend"]
    resource = {"cpu": 0, "rvv": 1, "npu": 2}[backend]
    flags = 0
    header = struct.pack("<IHHII", 0x31474541, 1, 1, 64, 0)
    segment = struct.pack("<HBBIII", 1, resource, flags, 0, 0, 64)
    path.write_bytes(header + segment)
    return {"magic": "AEG1", "version": 1, "segment_count": 1, "arena_size": 64, "segments": [{"id": 1, "resource": backend, "dependency_mask": 0, "arena_offset": 0, "arena_size": 64}]}
...
def main() -> int:
    ...
    plan = beam_plan(out / "cost.db", target)
    write_json(out / "plan.json", plan)
    write_json(out / "aeg_debug.json", emit_aeg(out / "model.aeg", plan))
    blocker = {
    ...
    report = {
        ...
        "npu_fallback_pass": blocker["status"] == "blocked" and plan["npu_fallback_enforced"],
        "beam_search_pass": plan["selected"]["segments"][0]["backend"] == "rvv",
